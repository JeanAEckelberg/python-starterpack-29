...
from game.character.action.attack_action import AttackActionType
...
def decide_move(character: Character, possible_moves: list[MoveAction], game_state: GameState, other_moves: list[MoveAction]) -> MoveAction | None:
    zombies: list[Character] = [c for c in game_state.characters.values() if c.is_zombie]  # filtering out for just zombie
    ...
    poses: list[Position] = [p.destination for p in possible_moves if not relative_dir_in_list(relative_direction(character.position, p.destination), dirs)]
    if len(poses) == 0:
...
def decide_ability(character: Character, possible_abilities: list[AbilityAction], game_state: GameState, other_abilities: list[AbilityAction]) -> AbilityAction | None:
    # wherever the closest zombie is in any of the 8 directions, place a barricade and keep running away
    # if character value is zombie
    # place barricade at where the closest zombie is
    zombies: list[Character] = [c for c in game_state.characters.values() if c.is_zombie] # filtering out for just zombie
    poses: list[Position] = [a.positional_target for a in possible_abilities]
    pos = closest(poses, zombies, manhattan_distance)
    if pos is None:
        return None
    to_return = [a for a in possible_abilities if pos == a.positional_target]
    if to_return is not None:
        to_return = to_return[0]
    return to_return
