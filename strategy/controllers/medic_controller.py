...
def decide_ability(character: Character, possible_abilities: list[AbilityAction], game_state: GameState, other_abilites: list[AbilityAction]) -> AbilityAction | None:
    pos_abilities = [x for x in possible_abilities if not game_state.characters.get(str(x.character_id_target)).is_zombie]
    if len(pos_abilities) == 0:
        return None
    return min(pos_abilities, key=lambda ability: game_state.characters.get(str(ability.character_id_target)).health)
